...
// 红线 R1：前端不自带认证——出现任何认证语义痕迹即失败
const re = /\b(password|passwd|oauth|bearer|jwt|secret|credential|authenticate|authorization)\b/i;
walk(path.join(POD, "frontend"), [".js", ".html"]).forEach(f => scan(f, re, "[R1]"));
if (hits.length) { console.error("FAIL R1 前端发现认证痕迹:\n" + hits.join("\n")); process.exit(1); }
console.log("PASS R1 前端无认证痕迹");
